...
| `BENCHMARK` | `.loop/<loop-id>/BENCHMARK.md` | search set, holdout set, expected-green controls, expected-red controls, metrics, timeouts |
| `CANDIDATES` | `.loop/<loop-id>/CANDIDATES.jsonl` | candidate rows, lineage, hypothesis, status, trace paths |
| `FRONTIER` | `.loop/<loop-id>/FRONTIER.json` | current best/Pareto members, evaluator health, pressure debt, checkpoint reason |
| `traces` | `.loop/<loop-id>/traces/<candidate>/<case>/evaluation.json` | raw per-candidate/per-case evidence |
